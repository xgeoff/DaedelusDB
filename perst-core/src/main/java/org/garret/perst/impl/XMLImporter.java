...
public class XMLImporter { 
    ...
    public void importDatabase() throws XMLImportException { 
        ...
        while ((tkn = scanner.scan()) != XMLScanner.XML_GT) {        
            if (storage.xmlImportReuseOid
                && tkn == XMLScanner.XML_IDENT 
                && scanner.getIdentifier().equals("maxoids")
                && scanner.scan() == XMLScanner.XML_EQ
                && scanner.scan() == XMLScanner.XML_SCONST)
            {                
        ...
        if (elem != null) {
            if (elem.isStringValue()) {
                ...
                if (name.equals("scalar")) { 
                    ...
                    String className = getAttribute(value, "name");
                ...
                case ClassDescriptor.tpDate:
                    ...
                    if (elem != null) { 
                        if (elem.isIntValue()) {
                            ...
                            Bytes.pack8(buf.arr, offs, Storage.INVALID_DATE);
                        } else if (elem.isStringValue()) {
