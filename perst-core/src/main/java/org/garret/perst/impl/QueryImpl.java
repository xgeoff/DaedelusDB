...
class BinOpNode extends Node { 
    ...
    boolean evaluateBool(FilterIterator t) {
        switch (tag) { 
          ...
          case opStrIgnoreCaseEq:
            return left.evaluateStr(t).equalsIgnoreCase(right.evaluateStr(t));
          case opStrIgnoreCaseNe:
            return !left.evaluateStr(t).equalsIgnoreCase(right.evaluateStr(t));
          case opStrIgnoreCaseLt:
            return left.evaluateStr(t).compareToIgnoreCase(right.evaluateStr(t)) < 0;
          case opStrIgnoreCaseLe:
            return left.evaluateStr(t).compareToIgnoreCase(right.evaluateStr(t)) <= 0;
          case opStrIgnoreCaseGt:
            return left.evaluateStr(t).compareToIgnoreCase(right.evaluateStr(t)) > 0;
          case opStrIgnoreCaseGe:
            return left.evaluateStr(t).compareToIgnoreCase(right.evaluateStr(t)) >= 0;
          ...
          case opByteArrayEq:
            return Arrays.equals((byte[])left.evaluateObj(t), (byte[])right.evaluateObj(t));
          case opByteArrayNe:
            return !Arrays.equals((byte[])left.evaluateObj(t), (byte[])right.evaluateObj(t));
...
class CompareNode extends Node {
    ...
    boolean evaluateBool(FilterIterator t) {
        switch(tag) { 
          ...
          case opStrIgnoreCaseBetween:
          {
            String val = o1.evaluateStr(t);
            return val.compareToIgnoreCase(o2.evaluateStr(t)) >= 0 
                && val.compareToIgnoreCase(o3.evaluateStr(t)) <= 0;
          }
          case opDateBetween:
          {
          ...
          case opStrIgnoreCaseLikeEsc:
          case opStrLikeEsc:
          {
            String str = o1.evaluateStr(t);
            String pat = o2.evaluateStr(t);
            if (tag == opStrIgnoreCaseLikeEsc) { 
                str = str.toLowerCase();
                pat = pat.toLowerCase();
            }
    ...
    final int compare(Object a, Object b) { 
        ...
        try {
            if (method != null) {
                ...
                case Node.tpDate:
                {
                    Date l = expr.evaluateDate(i1);
                    Date r = expr.evaluateDate(i2);
                    diff = l.compareTo(r);
                    break;
                }
                default:
                {
                    Comparable l = (Comparable)expr.evaluateObj(i1);
                    Comparable r = (Comparable)expr.evaluateObj(i2);
                    diff = l.compareTo(r);
                    break;
                }
                }                    
                ...
                switch (type) { 
                  ...
                  case ClassDescriptor.tpString:
                    if (isCaseInsensitive) { 
                        diff = ((String)field.get(a)).compareToIgnoreCase((String)field.get(b));
                        break;
                    }
                    // no break
                  default:
...
public class QueryImpl<T> implements Query<T> 
{
    ...
    {       
        ...
        try { 
            ...
            if (result == null) { 
                ...
                if (order == null || (result.key != null && order.field.equals(result.key) && order.next == null)) {
            ...
            if (storage.listener != null) { 
                storage.listener.queryExecution(cls, query, System.currentTimeMillis() - start, sequentialSearch);
            }
        }
    }
    ...
    final Node compare(Node expr, BinOpNode list)
    {
        ...
        do { 
            ...
            if (expr.type == Node.tpInt) { 
                ...
                cop = isCaseInsensitive(expr) ? Node.opStrIgnoreCaseEq : Node.opStrEq;
            } else if (expr.type == Node.tpObj && elem.type == Node.tpObj) {
                cop = Node.opObjEq;
            } else if (expr.type == Node.tpBool && elem.type == Node.tpBool) {
                cop = Node.opBoolEq;
            } else if (expr.type == Node.tpArrayInt1 && elem.type == Node.tpArrayInt1) {
                cop = Node.opByteArrayEq;
            ...
            || cop == tknLike || cop == tknNot || cop == tknIs || cop == tknIn)
        {
            ...
            if (cop == tknIn) { 
                ...
                if (right == singleElementList) { 
                    right = new BinOpNode(Node.tpList, Node.opNop, null, right);
                }
                ...
                {
                    left = new CompareNode(isCaseInsensitive(left) ? Node.opStrIgnoreCaseBetween : Node.opStrBetween, 
                                           left, right, right2);
                }
                ...
                if (lex == tknEscape) { 
                    ...
                    left = new CompareNode(isCaseInsensitive(left) ? Node.opStrIgnoreCaseLikeEsc : Node.opStrLikeEsc, 
                                           left, right,
                                           new StrLiteralNode(svalue));
                    lex = scan();
                } else { 
                    left = new CompareNode(isCaseInsensitive(left) ? Node.opStrIgnoreCaseLike : Node.opStrLike, left, right, null);
                }
            } else { 
                if (right.type == Node.tpUnknown) { 
                    right.type = left.type;
                ...
                if (left.type == Node.tpAny || right.type == Node.tpAny) {
                    ...
                    int strCmpOp = ((isCaseInsensitive(left) || isCaseInsensitive(right)) 
                                  ? Node.opStrIgnoreCaseEq : Node.opStrEq) + cop - tknEq;
                    left = new BinOpNode(Node.tpBool, strCmpOp, left, right);
                } else if (left.type == Node.tpDate) {
                    if (right.type == Node.tpStr) {
                        right = str2date(right);
                    } else if (right.type != Node.tpDate) {
                    ...
                    left = new BinOpNode(Node.tpBool, Node.opBoolEq+cop-tknEq, 
                                         left, right);
                } else if (left.type == Node.tpArrayInt1 && right.type == Node.tpArrayInt1) { 
                    if (cop != tknEq && cop != tknNe) { 
                        throw new CompileError("Byte array variables can be checked only for equality",
                                               rightPos);
                    }
                    left = new BinOpNode(Node.tpBool, Node.opByteArrayEq+cop-tknEq, 
                                         left, right);
                } else { 
            ...
            try { 
                ...
                if (m.isBridge()) {
                    ...
                    for (Method method : methods) {                        
                        if (method.getName() == ident && Arrays.equals(method.getParameterTypes(), profile) && !method.isBridge()) {
                            m = method;
                            break;
                        }   
                    }
            ...
            for (Class scope = cls; scope != null; scope = scope.getSuperclass()) { 
                try { 
                    ...
                    if (m.isBridge()) {
                        Method[] methods = scope.getDeclaredMethods();                    
                        for (Method method : methods) {                        
                            if (method.getName() == ident && method.getParameterTypes().length == 0  && !method.isBridge()) {
                                m = method;
                                break;
                            }   
                        }
    ...
    final Iterator tripleOpIndex(GenericIndex index, CompareNode cmp)
    {
        ...
        case Node.opAnyBetween:
        {
            ...
            if (value1 != null && value2 != null) { 
                return index.iterator(value1, value2, sort);
            }
            break;
        }
        case Node.opStrLike:
        ...
        case Node.opStrIgnoreCaseLikeEsc:
        {
            ...
            if (escape == '\\' && index instanceof RegexIndex && order == null) { 
                return ((RegexIndex)index).match(pattern);
            }
            int pref = 0;
            ...
            if (pref > 0) { 
                if (pref == pattern.length()) { 
                    ...
                    return index.iterator(value, value, sort);
                } else {
                    return index.prefixIterator(pattern.substring(0, pref), sort);
                }
            }
        }
        }
    ...
    final int getEqualsCost(Node node) { 
        ...
        int cost = Math.max(bin.left.getIndirectionLevel(), bin.right.getIndirectionLevel())*storage.sqlOptimizerParameters.indirectionCost;
    ...
    final int calculateCost(Node node) 
    {
        ...
        case Node.opContains:
            return params.containsCost + ((ContainsNode)node).withExpr.getIndirectionLevel()*params.indirectionCost;
        case Node.opBoolAnd:
            return params.andCost + Math.min(calculateCost(((BinOpNode)node).left), calculateCost(((BinOpNode)node).right));
        case Node.opBoolOr:
            return params.orCost + calculateCost(((BinOpNode)node).left) + calculateCost(((BinOpNode)node).right);
        case Node.opBoolEq:
        ...
        case Node.opIntBetween:
        case Node.opDateBetween:
            return params.closeIntervalCost + ((CompareNode)node).o1.getIndirectionLevel()*params.indirectionCost;
        case Node.opStrLike:
        case Node.opStrLikeEsc:
        case Node.opStrIgnoreCaseLike:
        case Node.opStrIgnoreCaseLikeEsc:
            return params.patternMatchCost + ((CompareNode)node).o1.getIndirectionLevel()*params.indirectionCost;
        case Node.opByteArrayEq:
        case Node.opObjEq:
        case Node.opIntEq:           
        case Node.opDateEq:           
        ...
        case Node.opIsNull:
            return params.isNullCost + ((UnaryOpNode)node).opd.getIndirectionLevel()*params.indirectionCost;
        case Node.opBoolNot:
            return params.eqBoolCost + params.notUniqCost + ((UnaryOpNode)node).opd.getIndirectionLevel()*params.indirectionCost;
        case Node.opLoad:
            return params.eqBoolCost + params.notUniqCost + node.getIndirectionLevel()*params.indirectionCost;
         default:
    ...
    final IndexSearchResult applyIndex(Node condition, Node predicate, Node filterCondition) 
    {
        ...
        switch (condition.tag) { 
            ...
            { 
                ...
                if (storage.sqlOptimizerParameters.enableCostBasedOptimization && calculateCost(and.left) > calculateCost(and.right)) { 
                    IndexSearchResult result = applyIndex(and.right, predicate, filterCondition == null ? and.left : predicate);
                    if (result != null) { 
                        return result;
                    }
                    return applyIndex(and.left, predicate, filterCondition == null ? and.right : predicate);
                } else {
                    IndexSearchResult result = applyIndex(and.left, predicate, filterCondition == null ? and.right : predicate);
                    if (result != null) { 
                        return result;
                    }
