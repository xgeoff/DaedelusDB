        ...
        for (int i = 0; i < fld.length; i++) {
            ...
            switch (types[i]) { 
              ...
              case ClassDescriptor.tpShort:
                ...
                break;
              case ClassDescriptor.tpChar:
                v = Character.valueOf((char)Bytes.unpack2(data, offs));
                offs += 2;
                break;
              case ClassDescriptor.tpInt:
              ...
              case ClassDescriptor.tpFloat:
                v = Float.valueOf(Float.intBitsToFloat(Bytes.unpack4(data, offs)));
                offs += 4;
                break;
              case ClassDescriptor.tpDouble:
            ...
            for (int i = 0; i < fld.length; i++) { 
                ...
                switch (types[i]) {
                  ...
                  case ClassDescriptor.tpDate:
                  {
                      ...
                      buf.extend(dst+8);
                      Bytes.pack8(buf.arr, dst, d == null ? Storage.INVALID_DATE : d.getTime());
                      dst += 8;
                      break;
                  }
                  case ClassDescriptor.tpFloat:
    ...
    private Key convertKey(Key key) { 
        ...
        for (int i = 0; i < values.length; i++) { 
            ...
            switch (types[i]) {
              ...
              case ClassDescriptor.tpDate:
                buf.extend(dst+8);
                Bytes.pack8(buf.arr, dst, v == null ? Storage.INVALID_DATE : ((Date)v).getTime());
                dst += 8;
                break;
              case ClassDescriptor.tpFloat:
                buf.extend(dst+4);
