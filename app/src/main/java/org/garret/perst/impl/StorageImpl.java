...
public class StorageImpl implements Storage {
    ...
    static final int  dbAlignedBitmapPageFlag = 8;
    static final int  dbPackedBitmapPageFlag = 16;

    static final int  dbPageFlagsMask = dbFlagsMask|dbAlignedBitmapPageFlag|dbPackedBitmapPageFlag;
    ...
    final Page getPage(int oid) {
        ...
        return pool.getPage(pos & ~dbPageFlagsMask);
    }
    ...
    final Page putPage(int oid) {
        synchronized (objectCache) {
            ...
            if ((pos & dbModifiedFlag) == 0) {
                ...
                cloneBitmap(pos & ~dbPageFlagsMask, Page.pageSize);
                pos = getPos(oid);
            }
            modified = true;
            return pool.putPage(pos & ~dbPageFlagsMask);
        }
    }
    ...
    final void freePage(int oid) {
        ...
        if ((pos & dbModifiedFlag) != 0) {
            free(pos & ~dbPageFlagsMask, Page.pageSize);
        } else {
            cloneBitmap(pos & ~dbPageFlagsMask, Page.pageSize);
        }
        freeId(oid);
    ...
    void reserveIds(int n) { 
       synchronized (objectCache) { 
            int curr = 1-currIndex;
            Assert.that(n > currIndexSize);
            setDirty();
            int oldIndexSize = header.root[curr].indexSize;
            if (n >= oldIndexSize) {
                ...
                } while (newIndexSize < n);
                
                long oldIndex = header.root[curr].index;
                long newIndex = allocate(newIndexSize*8L, 0);
                Assert.that(oldIndex == header.root[curr].index);
                pool.copy(newIndex, oldIndex, currIndexSize*8L);
                header.root[curr].index = newIndex;
                header.root[curr].indexSize = newIndexSize;
                free(oldIndex, oldIndexSize*8L);
    ...
    void collectFreeIds(int[] oids, int n) 
    {
        synchronized (objectCache) { 
            int curr = 1-currIndex;
            setDirty();
            for (int i = dbFirstUserId; i < n; i++) { 
                if (oids[i] == 0) { 
                    setPos(i, ((long)(header.root[curr].freeList) << dbFlagsBits) | dbFreeHandleFlag);
                    header.root[curr].freeList = i;
                }
            }
        }
    }
    ...
    final Page getBitmapPage(int i, int ignoreMask) {
        long pos = getPos(getBitmapPageId(i));
        if ((pos & (dbFreeHandleFlag|dbPageObjectFlag)) != dbPageObjectFlag) {
            throw new StorageError(StorageError.DELETED_OBJECT);
        }
        return (pos & ignoreMask) != 0 ? null : pool.getPage(pos & ~dbPageFlagsMask);
    }
            ...
            while (true) {
                if (alignment == 0) {
                    ...
                    for (i = firstPage; i < lastPage; i++){
                        ...
                        pg = getBitmapPage(i, dbPackedBitmapPageFlag);
                        if (pg == null) {
                            holeBitSize = 0;
                            offs = 0;
                            continue;
                        ...
                        while (offs < Page.pageSize) {
                            if (pg.data[offs++] != 0) {
                                ...
                                if (oid != 0) {
                                    ...
                                    int marker = (int)prev & ((prev & dbPageObjectFlag) != 0 ? dbPageFlagsMask : dbFlagsMask);
                    ...
                    for (i = firstPage; i < lastPage; i++){
                        ...
                        pg = getBitmapPage(i, dbAlignedBitmapPageFlag);
                        if (pg == null) {
                            holeBitSize = 0;
                            offs = 0;
                            continue;
                        ...
                        while (offs < Page.pageSize) {
                            ...
                            if (holeBitSize + BitmapAllocator.firstHoleSize[mask] >= objBitSize) {
                                ...
                                commitLocation();
                                return pos;
                            } else if (BitmapAllocator.maxHoleSize[mask] >= objBitSize) {
                                int holeBitOffset = BitmapAllocator.maxHoleOffset[mask];
                            ...
                            offs += 1;
                            if (BitmapAllocator.lastHoleSize[mask] == 8) {
                                holeBitSize += 8;
                            } else {
                                holeBitSize = BitmapAllocator.lastHoleSize[mask];
                            }
                        }
                        if (startOffs == 0 && holeBitSize == 0
                            && spaceNeeded < bitmapPageAvailableSpace[i])
                ...
                if (firstPage == 0 || reservedChainLength > dbAllocRecursionLimit) {
                    ...
                    j = i;
                    pos |= dbPageObjectFlag | dbModifiedFlag;
                    if (separateBitmap) {
                        pos |= alignment == 0 ? dbAlignedBitmapPageFlag : dbPackedBitmapPageFlag;
                    }
                    while (--morePages >= 0) {
                        setPos(getBitmapPageId(j++), pos);
                        pos += Page.pageSize;
                    }
     ...
     protected OidHashTable createObjectCache(String kind, long pagePoolSize, int objectCacheSize)
    {
        ...
        return pagePoolSize == INFINITE_PAGE_POOL
    ...
    public synchronized void open(IFile file, long pagePoolSize) {
        ...
        if (header.databaseFormatVersion == 0) { // database not initialized
            ...
            int pageFlags = dbPageObjectFlag | (separateBitmap ? dbAlignedBitmapPageFlag : 0);
            for (i = 0; i < bitmapPages; i++) {
                Bytes.pack8(index, (dbBitmapId+i)*8, used | pageFlags);
                used += Page.pageSize;
            }
    ...
    void reloadScheme() {
        ...
        customAllocatorSet = null;
        defaultAllocator = new DefaultAllocator(this);
        int descListOid = header.root[1-currIndex].classDescList;

        ClassDescriptor metaclass = new ClassDescriptor(this, ClassDescriptor.class);
        ClassDescriptor metafield = new ClassDescriptor(this, ClassDescriptor.FieldDescriptor.class);

        classDescMap.put(ClassDescriptor.class, metaclass);
        classDescMap.put(ClassDescriptor.FieldDescriptor.class, metafield);

        if ((compatibilityMode & IBM_JAVA5_COMPATIBILITY_MODE) != 0 && getDatabaseFormatVersion() == 1) {
            ...
            for (desc = descList; desc != null; desc = desc.next) {
                ...
                if (desc.allocator != null) {
                    if (customAllocatorMap == null) {
                        customAllocatorMap = new HashMap<Class,CustomAllocator>();
                        customAllocatorSet = new HashSet<CustomAllocator>();
                    }
                    CustomAllocator allocator = desc.allocator;
                    allocator.load();
                    customAllocatorMap.put(desc.cls, allocator);
                    customAllocatorSet.add(allocator);
    ...
    public void backup(String filePath, String cryptKey) throws IOException
    {
        backup(new IFileOutputStream(cryptKey != null
                                     ? (IFile)new Rc4File(filePath, false, false, cryptKey)
    ...
    public synchronized <T> IPersistentSet<T> createSet() {
        ...
        IPersistentSet<T> set = alternativeBtree
            ? (IPersistentSet<T>)new AltPersistentSet<T>(true)
    ...
    public synchronized <T> IPersistentSet<T> createBag() {
        ...
        IPersistentSet<T> set = alternativeBtree
            ? (IPersistentSet<T>)new AltPersistentSet<T>(false)
    ...
    public synchronized <T> FieldIndex<T> createFieldIndex(Class type, String fieldName, boolean unique, boolean caseInsensitive, boolean thick)
    {
        ...
        FieldIndex<T> index = thick
            ? caseInsensitive
                ? (FieldIndex<T>)new ThickCaseInsensitiveFieldIndex(this, type, fieldName)
                : (FieldIndex<T>)new ThickFieldIndex(this, type, fieldName)
            : caseInsensitive
                ? alternativeBtree
    ...
    public synchronized HashMap<Class,MemoryUsage> getMemoryDump() {
        synchronized (objectCache) {
            ...
            if (rootOid != 0) {
                ...
                if (customAllocatorSet != null) {
                    MemoryUsage custom = new MemoryUsage(CustomAllocator.class);
                    map.put(CustomAllocator.class, custom);
                    for (CustomAllocator allocator : customAllocatorSet) {
                        custom.totalSize += allocator.getAllocatedMemory();
                        custom.allocatedSize += allocator.getUsedMemory();
                        custom.nInstances += 1;
                    }
                }
    ...
    final int markObject(byte[] obj, int offs,  ClassDescriptor desc)
    {
        ...
        Assert.that(!SelfSerializable.class.isAssignableFrom(desc.cls));
    ...
    public void setProperties(Properties props)
    {
        ...
        if ((value = props.getProperty("perst.separate.bitmap")) != null) {
            separateBitmap = getBooleanValue(value);
        }
        ...
        if ((value = props.getProperty("perst.reload.objects.on.rollback")) != null) {
            reloadObjectsOnRollback = getBooleanValue(value);
        }
        if ((value = props.getProperty("perst.reuse.oid")) != null) {
            reuseOid = getBooleanValue(value);
        }
        if ((value = props.getProperty("perst.xml.reuse.oid")) != null) {
            xmlImportReuseOid = getBooleanValue(value);
        }
        if ((value = props.getProperty("perst.serialize.system.collections")) != null) {
            serializeSystemCollections = getBooleanValue(value);
        }
        if ((value = props.getProperty("perst.compatibility.mode")) != null) {
            compatibilityMode = (int)getIntegerValue(value);
        }
        if ((value = props.getProperty("perst.ignore.missed.classes")) != null) {
            ignoreMissedClasses = getBooleanValue(value);
        }
    ...
    public void setProperty(String name, Object value)
    {
        ...
        if (name.equals("perst.implicit.values")) {
            ...
            noFlush = getBooleanValue(value);
        } else if (name.equals("perst.xml.date.format")) {
            xmlDateFormat = (String)value;
        } else if (name.equals("perst.alternative.btree")) {
            alternativeBtree = getBooleanValue(value);
        } else if (name.equals("perst.separate.bitmap")) {
            separateBitmap = getBooleanValue(value);
            ...
            reuseOid = getBooleanValue(value);
        } else if (name.equals("perst.xml.reuse.oid")) {
            xmlImportReuseOid = getBooleanValue(value);
        } else if (name.equals("perst.compatibility.mode")) {
            compatibilityMode = (int)getIntegerValue(value);
    ...
    final int packValue(Object value, int offs, ByteBuffer buf) throws Exception {
        if (value == null) {
            ...
            if (c == Boolean.class) {
                ...
                ObjectOutputStream out = loaderMap != null && (compatibilityMode & CLASS_LOADER_SERIALIZATION_COMPATIBILITY_MODE) == 0
    ...
    protected boolean ignoreMissedClasses = false;
    protected boolean xmlImportReuseOid = false;
    ...
    protected boolean separateBitmap = false;
    protected String  xmlDateFormat = "EEE, d MMM yyyy kk:mm:ss z";

    private HashMap<Class,CustomAllocator> customAllocatorMap;
    private HashSet<CustomAllocator> customAllocatorSet;
